...
class LoginModel
{
    ...
    public function login()
    {
        ...
        if (password_verify($_POST['user_password'], $result->user_password_hash)) {
            ...
            $arr = array();
            $arr[] = array('success' => 1, 'user_name' => $user_name, 'user_email' => $user_email);
    ...
    /*
        This function returns the list of tracking items the user has
    */
    public function getUserItems(){
        ...
        $item_DOB = NULL;
        $item_lastTracked = NULL;
        $item_picture = NULL;
        $item_isLost = NULL;
        $item_eLeashOn = NULL;
        $item_eLeashType = NULL;
        $item_eLeashRange = NULL;
        $item_macAddress = NULL;
        //var_dump($sth->fetch());

        //Bind the output to the variables
        /*if(!$sth->bind_result($item_id, $item_name, $item_description, $item_DOB, $item_lastTracked, $item_picture, $item_isLost, $item_eLeashOn, $item_eLeashType, $item_eLeashRange, $item_macAddress)){
            $_SESSION["feedback_negative"][] = FEEDBACK_USERDATA_NOT_RETRIVED;
            return false;
        }*/
        while ($row=$sth->fetch())
        {
            ...
            $item_eLeashType = $row->item_eLeashType;
            $item_eLeashRange = $row->item_eLeashRange;
            $item_macAddress = $row->item_macAddress;

            $arr[] = array('item_name' => $item_name, 'item_description' => $item_description, 'item_DOB' => $item_DOB, 'item_lastTracked' => $item_lastTracked, 'item_picture' => $item_picture, 'item_isLost' => $item_isLost, 'item_eLeashOn' => $item_eLeashOn, 'item_eLeashType' => $item_eLeashType, 'item_eLeashRange' => $item_eLeashRange, 'item_macAddress' => $item_macAddress);
        }
        return $arr;
    }

    /**
    *   This function adds a new item to the list of exising user items
    *
    */
    public function addUserItem(){
        ...
        if (empty($_POST['user_name'])) {
        ...
        } elseif (empty($_POST['item_macAddress'])) {
            $_SESSION["feedback_negative"][] = FEEDBACK_ITEM_DOB_FIELD_EMPTY;
        } elseif (empty($_POST['item_DOB'])) {
            $_SESSION["feedback_negative"][] = FEEDBACK_ITEM_MAC_ADDRESS_FIELD_EMPTY;
            ...
            $new_item_name = strip_tags($POST['new_item_name']);
            $item_description = strip_tags($_POST['item_description']);

            $item_isLost = $_POST['item_isLost'];
            $item_eLeashRange = $_POST['item_eLeashRange'];
            $item_eLeashOn = $_POST['item_eLeashOn'];
            $item_macAddress = $_POST['item_macAddress'];
            $item_lastTracked = $_POST['item_lastTracked'];
            $item_DOB = $_POST['item_DOB'];


            //Upload image file to server if it exists
            $item_picture= NULL;
            if(!empty($_FILES['imageFile'])){
                $destination_path = getcwd().DIRECTORY_SEPARATOR;
                $target_path = $destination_path . IMAGE_PATH . basename( $_FILES['imageFile']['name']);

                $item_picture =  basename( $_FILES['imageFile']['name']);
                //First check if image data exists, i.e. was sent via some method here
            ...
            //Execute the call to the database
            $sth->execute(array(':user_name' => $_POST['user_name'], ':item_name' => $POST['item_name']));
            ...
            if($count==0){
                ...
                $sql = "UPDATE items SET user_name=:user_name, item_name=:new_item_name, item_description=:item_description, item_DOB=:item_DOB,  item_lastTracked=:item_lastTracked, item_picture=:item_picture, item_isLost= :item_isLost, item_eLeashOn=:item_eLeashOn, item_eLeashRange=:item_eLeashRange, item_macAddress=:item_macAddress
            ...
            $query->execute(array(':user_name' => $user_name,
                                  ...
                                  ':item_DOB' => $item_DOB,
                                  ':item_lastTracked' => $item_lastTracked,
...
/**
    *   This function deletes an item from the list of exising user items
    *
    */
    public function deleteUserItem(){
        ...
        if (empty($_POST['user_name'])) {
            ...
            // get user's item data
            $sth = $this->db->prepare("DELETE from items
                                       WHERE  user_name = :user_name and item_name = :item_name");
            //Execute the call to the database
            ...
            $count =  $sth->rowCount();
            if ($count != 1) {
                $_SESSION["feedback_negative"][] = FEEDBACK_ITEM_DELETION_FAILED;
                return false;
            }
            return true;
    ...
    */
    public function addUserBeacon(){
        ...
        if (empty($_POST['user_name'])) {
            ...
            if ($count==0){
                ...
                $sql = "UPDATE beacon SET user_name=:user_name, item_name=:new_item_name, uuid=:uuid, major=:major,  minor=:minor, event=:event, action= :action, message=:message
                        WHERE user_name = :user_name1 and item_name = :item_name1";
            }
            $query = $this->db->prepare($sql);
            $query->execute(array(':user_name' => $user_name,
    ...
    /**
    *   This function deletes a beacon from the list of exising user beacons
    *
    */
    public function deleteUserBeacon(){
        ...
        if (empty($_POST['user_name'])) {
            ...
            // get user's item data
            $sth = $this->db->prepare("DELETE from beacon
                                       WHERE  user_name = :user_name and item_name = :item_name");
            //Execute the call to the database
            $sth->execute(array(':user_name' => $_POST['user_name'],':item_name' => $_POST['item_name']));
            ...
            $count =  $sth->rowCount();
            if ($count != 1) {
                $_SESSION["feedback_negative"][] = FEEDBACK_BEACON_DELETION_FAILED;
                return false;
            }
            return true;
    ...
    private function sendVerificationEmail($user_id, $user_email, $user_activation_hash)
    {
        ...
        $headers["From"] = EMAIL_VERIFICATION_FROM_EMAIL;
        $headers["To"] = $user_email;
        $headers["Subject"] = EMAIL_VERIFICATION_SUBJECT;
        $mailmsg = EMAIL_VERIFICATION_CONTENT . EMAIL_VERIFICATION_URL . '/' . urlencode($user_id) . '/' . urlencode($user_activation_hash);
        //SMTP server name, port, user/passwd 
        $smtpinfo["host"] = EMAIL_SMTP_HOST;
        $smtpinfo["port"] = EMAIL_SMTP_PORT;
        $smtpinfo["auth"] = EMAIL_SMTP_AUTH;
        $smtpinfo["username"] = EMAIL_SMTP_USERNAME;
        $smtpinfo["password"] = EMAIL_SMTP_PASSWORD;
        ...
        if($mail_object->send($recipients, $headers, $mailmsg)){
            ...
            $_SESSION["feedback_negative"][] = FEEDBACK_VERIFICATION_MAIL_SENDING_ERROR;
            return false;
        }*/
    }
